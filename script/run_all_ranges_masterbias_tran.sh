...
for (( r=RANGE_START; r<=RANGE_END; r++ )); do
  ...
  bash "${SCRIPT_DIR}/run_ngspice_parallel_masterbias_tran.sh" \
  ...
  for (( s=0; s<MC_SAMPLES; s++ )); do
    ...
    while IFS= read -r f; do
      BATCH_RAWS+=("$f")
    done < <(ls "${OUT_DIR}/start_up_${r}_"*".raw" 2>/dev/null)

    if (( ${#BATCH_RAWS[@]} == 0 )); then
      echo "ERROR: no .raw files found matching: ${OUT_DIR}/start_up_${r}_*.raw"
      continue
    fi
