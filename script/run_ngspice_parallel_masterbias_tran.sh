...
for (( start=0; start<TOTAL_CASES; start+=BATCH_SIZE )); do
  ...
  BATCH_SPICE="${OUT_DIR}/${BASENAME}_start_up_${INITIAL_RANGE}_batch${BATCH}.${EXT}"
  BATCH_LOG="${OUT_DIR}/${BASENAME}_start_up_${INITIAL_RANGE}_batch${BATCH}.log"
  BATCH_LOGS+=("$BATCH_LOG")
  BATCH_STARTS+=($start)
  BATCH_ENDS+=($end)

  cp "$SPICE_FILE" "$BATCH_SPICE"
  sed -i "43s/.*/let initial_range =${INITIAL_RANGE}/" "$BATCH_SPICE"
  sed -i "45s/.*/let max_range = ${INITIAL_RANGE} + 1/" "$BATCH_SPICE"
  sed -i "54s/.*/let min =${start}/" "$BATCH_SPICE"
  sed -i "55s/.*/let max =${end}/"   "$BATCH_SPICE"
...
show_progress() {
  ...
  for (( b=0; b<NUM_BATCHES; b++ )); do
    ...
    local completed=0
    local raw_files
    raw_files=( "${OUT_DIR}/start_up_${INITIAL_RANGE}_${bmin}_"*.raw )
