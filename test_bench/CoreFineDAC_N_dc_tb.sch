...
.options abstol=5e-15
.options gmin=5e-15
.options gmindc=5e-15
.control
let V_drain_ISINK_vect = vector(16)
let initial_range = 1
let code_range = initial_range
let max_range = initial_range +1
let digitalCommand = vector(16)
let out_UP = vector(16)
let V_out_UP = vector(16)
while code_range < max_range
    * Set the 3 bits
    ...
    while code < max 
      ...
      * Set the 8 bits
      ...
      let digitalCommand[code-min] = code
      let out_UP[code-min] = I(V.x1.V_out_UP)
      let V_out_UP[code-min] = V(bias_VP)
      let V_drain_ISINK_vect[code-min] = V(x1.V_drain_ISINK)
      * let out_UP[code-min] = I(V.x1.V_Ibias1)
      let code = code + 1
      print code
   ...
   write  simulations/CoreFineDAC_N_dc_tb/commandRange_\{$&initial_range\}_\{$&max\}.raw out_UP V_out_UP V_drain_ISINK_vect
   *write CoreFineDAC_N_dc_tb/commandRange_\{$&initial_range\}_\{$&max\}.raw out_UP
end
